def _task(client, titel="Testtask", tags=None):
    r = client.post("/tasks", json={"titel": titel, "tags": tags or []})
...
def test_unbekanntes_tag_wird_abgelehnt(client):
    r = client.post("/tasks", json={"titel": "X", "tags": ["dringend"]})
    assert r.status_code == 422

    task = _task(client)
    r = client.put(f"/tasks/{task['id']}/tags/dringend")
    assert r.status_code == 422
...
def test_tag_setzen_und_entfernen(client):
    task = _task(client)
    r = client.put(f"/tasks/{task['id']}/tags/aktiv")
    ...
    r = client.delete(f"/tasks/{task['id']}/tags/aktiv")
    assert r.json()["tags"] == []


def test_tag_historie(client):
    task = _task(client, tags=["next"])
    client.put(f"/tasks/{task['id']}/tags/aktiv")
    client.delete(f"/tasks/{task['id']}/tags/next")

    r = client.get(f"/tasks/{task['id']}/historie")
...
def test_filter_nach_tag(client):
    ...
    r = client.get("/tasks", params={"tag": "aktiv"})
    assert [t["id"] for t in r.json()] == [aktiv["id"]]

    r = client.get("/tasks", params={"tag": "quatsch"})
    assert r.status_code == 422


def test_task_aendern_und_loeschen(client):
    task = _task(client)
    r = client.patch(f"/tasks/{task['id']}", json={"beschreibung": "Mehr Kontext"})
...
def test_queue_umsortieren(client):
    ...
    r = client.put("/queue/order", json={"task_ids": [c, a, b]})
    assert r.status_code == 200
...
def test_umsortieren_braucht_alle_ids(client):
    ...
    r = client.put("/queue/order", json={"task_ids": [a]})
    assert r.status_code == 422

    r = client.put("/queue/order", json={"task_ids": [a, a]})
    assert r.status_code == 422
