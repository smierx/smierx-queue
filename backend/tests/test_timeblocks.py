...
def test_bereichsfilter_zaehlt_ueberlappung(client):
    ...
    r = client.get("/api/timeblocks", params={"von": "2026-07-14", "bis": "2026-07-14"})
...
def test_block_aendern_und_loeschen(client):
    ...
    r = client.delete(f"/api/timeblocks/{block['id']}")
    assert r.status_code == 204
    assert client.get("/api/timeblocks").json() == []
